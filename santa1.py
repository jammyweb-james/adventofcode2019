...
def filla(array, ins, w):
        ...
        if direction == "R":
            ...
            for f in range(y, y-distance, -1):
                if array[x, f] == 0:
                    ...
                    crosses.append([x,f])
            y = y - distance
        if x > len(array) or x < 0 or y > len(array) or y < 0:
            print("out of range")
    #     maxx = max(x, maxx)
    #     minx = min(x, minx)
    #     maxy = max(y, maxy)
    #     miny = min(y, miny)
    # output = "x={}..{} y={}..{} === "
    # print(output.format(minx, maxx, miny, maxy))
    return crosses

def findsmallestmanhat(size, crosses):
    center = int(size/2)
    minman = center
    for cross in crosses:
        manhat = abs(center - cross[0]) + abs(center - cross[1])
        if manhat != 0:
            minman = min(minman, manhat)
    return minman

def countSteps(array, crosses, ins):
    x = int(len(array)/2)
    y = x
    steps = 0
    # array should have 1, 2 or 3
    # when array has 3 - mark cross with steps
    for i in ins:
        direction = i[0]
        distance = int(i[1:])
        if direction == "R":
            for f in range(x, x+distance):
                if array[f, y] == 3:
                    # do something with crosses
                    print(f, y)       
                    p = crosses.index([f, y])
                    if len(crosses[p]) == 2:
                        crosses[p].append(steps)             
                    else:
                        crosses[p][2] = crosses[p][2] + steps
                steps = steps + 1
            x = x + distance
        elif direction == "L":
            for f in range(x, x-distance, -1):
...
def day3():
    ...
    wire2a = wire2.split(",")
    path = {}
    path = traceWire(path, wire1a, 1)
    path = traceWire(path, wire2a, 2)
    size = 30000
    a = np.zeros((size,size),dtype=np.int8)
    crosses = filla(a,wire1a,1)
    crosses = filla(a,wire2a,2)
    center = int(size/2)
    print(a[center, center])
    minman = findsmallestmanhat(size, crosses)
    print(minman)
    countSteps(a, crosses, wire1a)
    countSteps(a, crosses, wire2a)
    print("done")
